...
5. **Display only** — the tool **never** makes changes in the Cordial or Braze UI. It renders code blocks with per-block Copy buttons and a Copy-all button, alongside a manual implementation guide of the steps the developer performs themselves.
...
| Atlassian     | Reads linked Confluence pages and searches Confluence for context             |
